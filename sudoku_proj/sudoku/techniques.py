        ...
        for i in range(1, 10):
            for j in range(i + 1, 10):
                for k in range(j + 1, 10):
                    locations_set.clear()
                    for loc in digit_locations[i]:
                        locations_set.add(loc)
                    for loc in digit_locations[j]:
                        locations_set.add(loc)
                    for loc in digit_locations[k]:
                        locations_set.add(loc)
                    if len(digit_locations[i]) > 0 and len(digit_locations[j]) > 0 and \
                            len(digit_locations[k]) > 0 and len(locations_set) == 3:
                        for x in locations_set:
                            for digit in copy.copy(solving_puzzle[row][x]):
                                if digit != i and digit != j and digit != k:
                                    if hints:
        ...
        for i in range(1, 10):
            for j in range(i + 1, 10):
                for k in range(j + 1, 10):
                    ...
                    if len(digit_locations[i]) > 0 and len(digit_locations[j]) > 0 and \
                            len(digit_locations[k]) > 0 and len(locations_set) == 3:
                        for y in locations_set:
                            for digit in copy.copy(solving_puzzle[y][col]):
                                if digit != i and digit != j and digit != k:
                                    if hints:
        ...
        for i in range(1, 10):
            for j in range(i + 1, 10):
                for k in range(j + 1, 10):
                    ...
                    if len(digit_locations[i]) > 0 and len(digit_locations[j]) > 0 and \
                            len(digit_locations[k]) > 0 and len(locations_set) == 3:
                        for row, col in locations_set:
                            for digit in copy.copy(solving_puzzle[row][col]):
                                if digit != i and digit != j and digit != k:
                                    if hints:
                                        return 'There is a Hidden Triplet in Block ' + str(get_block(row, col) + 1)
                                    else:
                                        solving_puzzle[row][col].remove(digit)
                                        progress = True
                        if progress:
...
def hidden_quad(solving_puzzle, hints=False):
            ...
            for j in range(i + 1, 10):
                for k in range(j + 1, 10):
                    for m in range(k + 1, 10):
                        ...
                        for loc in digit_locations[k]:
                            locations_set.add(loc)
                        for loc in digit_locations[m]:
                            locations_set.add(loc)
                        if len(digit_locations[i]) > 0 and len(digit_locations[j]) > 0 and \
                                len(digit_locations[k]) > 0 and len(digit_locations[m]) > 0 and \
                                len(locations_set) == 4:
                            for x in locations_set:
                                for digit in copy.copy(solving_puzzle[row][x]):
                                    if digit != i and digit != j and digit != k and digit != m:
                                        if hints:
                                            return 'There is a Hidden Quad in Block ' + str(get_block(row, x) + 1)
                                        else:
                                            solving_puzzle[row][x].remove(digit)
                                            progress = True
                            if progress:
                        ...
                        if len(digit_locations[i]) > 0 and len(digit_locations[j]) > 0 and \
                            ...
                            for y in locations_set:
                                for digit in copy.copy(solving_puzzle[y][col]):
                                    if digit != i and digit != j and digit != k and digit != m:
                                        if hints:
                                            return 'There is a Hidden Quad in Block ' + str(get_block(y, col) + 1)
                                        else:
                                            solving_puzzle[y][col].remove(digit)
                                            progress = True
                            if progress:
    ...
    for block in range(9):
        ...
        # finished with the block, look for 4 digits that only exist in same 4 cells
        for i in range(1, 10):
            for j in range(i + 1, 10):
                for k in range(j + 1, 10):
                    for m in range(k + 1, 10):
                        ...
                        if len(digit_locations[i]) > 0 and len(digit_locations[j]) > 0 and \
                            ...
                            for row, col in locations_set:
                                for digit in copy.copy(solving_puzzle[row][col]):
                                    if digit != i and digit != j and digit != k and digit != m:
                                        if hints:
                                            return 'There is a Hidden Quad in Block ' + str(get_block(row, col) + 1)
                                        else:
                                            solving_puzzle[row][col].remove(digit)
                                            progress = True
                            if progress:
